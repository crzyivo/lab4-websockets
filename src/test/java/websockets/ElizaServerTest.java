...
public class ElizaServerTest {
	...
	public void onChat() throws DeploymentException, IOException, URISyntaxException, InterruptedException {
		String sorryResponse = "Please don't apologize.";
		List<String> list = new ArrayList<>();
		ClientEndpointConfig configuration = ClientEndpointConfig.Builder.create().build();
		ClientManager client = ClientManager.createClient();
		...
		assertEquals(sorryResponse,list.get(0));
	}
    ...
    private static class ElizaEndpointToComplete extends Endpoint {
        ...
        private class ElizaMessageHandlerToComplete implements MessageHandler.Whole<String> {
            ...
            public void onMessage(String message) {
                LOGGER.info("Client received \""+message+"\"");
                if(!(message.contains("-")||message.contains("doctor is in")||message.contains("on your mind"))) {
                    list.add(message);
                    synchronized (list) {
                        list.notify();
                    }
